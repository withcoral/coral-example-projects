...
Both the `#alerts` auto-investigation path and the `@`-mention follow-up
path produce the same shape of reply, using Slack's AI-agent Block Kit
primitives:

1. *Contextual quick-ack* — a `:mag:` one-liner generated by a fast no-tools
   model call (~1s) so the channel sees the bot understood the request:
   `:mag: Investigating elevated error rate on hello-service — 3 exceptions in the last 5m`.
2. *Live plan block* — `chat.startStream` opens a streaming message; each
   Coral MCP tool call becomes a `TaskUpdateChunk` (`in_progress` → `complete`
   or `error` if the model is asked to retry). Same `task_id` patches in place,
   so the block stays expanded as it updates.
3. *Final assessment* — `chat.stopStream` finalizes with:
   - `alert` block (severity banner: error / warning / success)
   - `markdown` block carrying the GitHub-flavored Markdown body (renders as
     proper `## headers`, GFM `tables`, fenced code with language hints,
     `[link](url)` syntax)
   - `header` + `actions` blocks with **one URL button per source** (Datadog,
     Sentry, GitHub) so the operator can open the underlying system in one click
   - `context` footer with model name, tool-call count, and wall-clock
     duration: `:robot_face: claude-opus-4-7 · :wrench: 18 Coral queries · :stopwatch: 47s`
